"""Tests the open question from Experiment 13 (docs/RESEARCH_LOG.md):
LayerNorm amplifies the Transformer's post-training behavioral-robustness
gain 2-4x, but *shrinks* the MLP's gain ~3.3x -- the same rank-decoupling
effect generalizes across architectures, but the magnitude-sign does not.
Attention (present in the Transformer, absent in the MLP) is the obvious
untested candidate for what determines that sign.

Trains the Stage C-lite Transformer in 4 conditions (attention x
layernorm, each on/off; residual connections held on throughout, since
Experiment 11 found they don't matter much) and measures, per condition,
the mean behavioral-robustness gain and its rank-shrinkage correlation --
the same methodology as Experiments 11 and 13.

If disabling attention makes LayerNorm's effect on gain magnitude flip
toward the MLP's (shrinking rather than amplifying), that confirms
attention is the necessary ingredient for the sign. If LayerNorm still
amplifies the gain without attention, something else about the
Transformer (not attention specifically) is responsible instead.
"""
...
CONDITIONS = [
    ("attention_and_layernorm", True, True),
    ("no_attention", False, True),
    ("no_layernorm", True, False),
    ("no_attention_no_layernorm", False, False),
]
...
def main() -> None:
    ...
    for condition_name, use_attention, use_layernorm in CONDITIONS:
        for seed in SEEDS:
            ...
            model = build_ablation_transformer_classifier(
                ...
                use_residual=True, use_layernorm=use_layernorm, use_attention=use_attention,
            )
            ...
            for layer_name in linear_layer_names(model):
                ...
                row_random = run_layer_experiment(
                    ...
                    experiment_name="atlas_nn-stage_c_lite_attention_ablation",
                )
    ...
    save_json(
        {
            "experiment": "atlas_nn-stage_c_lite_attention_ablation",
            "seeds": list(SEEDS),
            "conditions": [c[0] for c in CONDITIONS],
            "summary": summary,
            "rows": all_rows,
        },
        "results/atlas_nn_stage_c_lite_attention_ablation.json",
    )
