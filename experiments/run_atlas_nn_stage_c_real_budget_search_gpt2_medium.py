"""Experiment 23: the gpt2-medium counterpart to Experiments 19/21's
budget searches on distilgpt2/gpt2 -- the first trustworthy (budget-
search, not fixed-method) depth-gradient reading for a third real
pretrained model, testing whether gpt2-medium (355M, 24 blocks, same
non-distilled training recipe as gpt2) matches gpt2's pattern (supporting
the training-procedure hypothesis for Experiment 21's reversal) or
diverges yet again (supporting a scale/depth-based explanation instead).

Uses atlas_nn.stage_c_real.parallel_budget_search (verified against the
sequential version in experiments/verify_parallel_budget_search.py)
rather than the sequential run_state loop, since gpt2-medium's larger
matrices make a full sequential budget search impractically slow on this
container's 4 CPU cores.
...
OUTPUT_PATH = "results/atlas_nn_stage_c_real_budget_search_gpt2_medium.json"
