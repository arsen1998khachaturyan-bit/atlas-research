"""Disentangles Experiment 14's attention-ablation result. `NoMixingAttention`
(used there) simultaneously removes cross-token mixing AND ~3/4 of
attention's parameters (no in_proj_weight) -- so the 79% gain drop found
when disabling attention could have been about either factor, or both.

This compares three conditions (LayerNorm and residual connections held
on throughout, matching Experiment 14's most informative contrast):
  - full_attention: real nn.MultiheadAttention (baseline)
  - no_mixing_no_param_match: Experiment 14's original NoMixingAttention
    (no mixing AND fewer params) -- included to confirm this script
    reproduces Experiment 14's numbers before trusting the new condition
  - no_mixing_matched_params: MatchedParamNoMixingAttention (no mixing,
    but same total parameter count as real attention)

If `no_mixing_matched_params` lands close to `no_mixing_no_param_match`
(both far below `full_attention`), the gain-magnitude effect is really
about cross-token mixing, not parameter count. If it lands close to
`full_attention` instead, parameter count was the real factor all along.
"""
...
CONDITIONS = [
    ...
    ("no_mixing_no_param_match", False, False),
    ("no_mixing_matched_params", False, True),
]
...
def main() -> None:
    ...
    for condition_name, use_attention, match_params in CONDITIONS:
        for seed in SEEDS:
            ...
            model = build_ablation_transformer_classifier(
                ...
                use_attention=use_attention, match_no_mixing_params=match_params,
            )
            ...
            for layer_name in linear_layer_names(model):
                ...
                row_random = run_layer_experiment(
                    ...
                    experiment_name="atlas_nn-stage_c_lite_attention_param_disentangle",
                )
                ...
                all_rows.append({
                    ...
                    "match_no_mixing_params": match_params,
                    "layer": layer_name,
                    "seed": seed,
    ...
    save_json(
        {
            "experiment": "atlas_nn-stage_c_lite_attention_param_disentangle",
            "seeds": list(SEEDS),
            "conditions": [c[0] for c in CONDITIONS],
            "summary": summary,
            "rows": all_rows,
        },
        "results/atlas_nn_stage_c_lite_attention_param_disentangle.json",
    )
